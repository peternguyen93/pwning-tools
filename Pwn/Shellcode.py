...
class Shellcode(str):
	def __new__(cls,content,arch = 'x86',address=0x1000): # default architecture
	...
	def pdisas(self):
		...
		if not md:
			...
			for (address, size, mnemonic, op_str) in md.disasm_lite(str.__str__(self), self.address):	
	...
	def disas(self):
		...
		if not md:
			...
			out = ''
			for (address, size, mnemonic, op_str) in md.disasm_lite(str.__str__(self), self.address):
				asm_str = '0x%x: %s %s;' % (address, mnemonic, op_str)
				out += asm_str
			out = out[:-1]
			return out
